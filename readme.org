...
#+begin_src bash
biber --tool --configfile=biberConf.xml tensor_methods/references.bib --output-file refsTmp.bib --output-legacy-date --output-field-replace=location:address,journaltitle:journal
#+end_src
*** Images
